...
function LeftBar({alertAnalysisStore, monitorAlertStore, routing, companyHomeStore}) {
  ...
  const nextClick = () => {
    const maxNum = moduleData.info.alertType !== 'SYS_RULE' ? dataStore.detailData.orgData.totalElements : data.length;
    if (page * 8 >= maxNum) {
      return false;
    }
    if (moduleData.info.alertType !== 'SYS_RULE') {
  ...
  return (
    <div className={styles.tabBox}>
      ...
      <div className={styles.overBox}>
        <div className={styles.itemBox} style={{top: moduleData.info.alertType !== 'SYS_RULE' ? 0 : tabTop}}>
          {createTabs()}
        </div>
      </div>
