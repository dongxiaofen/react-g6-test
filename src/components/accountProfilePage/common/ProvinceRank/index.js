...
function ProvinceRank({myProvinceRank}) {
  const moduleData = myProvinceRank;
  ...
  const getDataMinMax = () => {
    const data = moduleData.map(item => item.total);
  ...
  return (
    <div className={styles.box}>
      <span className={styles.titleText}>地域分布</span>
      <Chart
        chartId="myProvinceRank"
        height="440"
        option={chartOption} />
...
ProvinceRank.propTypes = {
  myProvinceRank: PropTypes.object,
};
export default loadingComp({
  mapDataToProps: props => ({
    loading: props.myProvinceRank.length > 0 ? false : true,
    error: props.myProvinceRank.error,
    height: 500,
    errCategory: 1,
    category: 0,
  }),
